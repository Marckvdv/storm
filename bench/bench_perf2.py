...
amounts = [ int(10**x) for x in range(0, 4) ]
priors = [ 10**x for x in [0, -1, 1, -3, 3] ]
avg_count = 10
...
for model in models:
    ...
    for amount in amounts:
        for prior in priors:
            ...
            total_error = 0
            total_branches = 0
            print(f'prior={prior}, runs={amount}, rounds={rounds}:', end=' ', flush=True)
            for _ in range(avg_count):
                params = [bin_path]
                params += model['extra_params']
                params += ['--generateObservations', '--runs', str(amount), '--rounds', str(rounds), '--generateUncertainMdp', '--priorInit', str(prior)]
